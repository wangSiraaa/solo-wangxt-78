...
`internal/store/migrations/` 下的增量迁移按文件名序执行，已应用版本记录在
`schema_migrations` 表，可重复执行（已应用的自动跳过），每个迁移在独立事务中
完成。**0001 不再改写**：0002 起以增量方式演进（补列、回填、约束、索引），
旧库（bcc7c845 schema）原地升级且保留全部数据与投递状态；集成测试
`TestUpgradeFromBcc7c845` 用冻结的旧 schema 夹具验证原地升级。
